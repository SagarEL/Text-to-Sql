                              ...
                              <Dialog>
                                ...
                                <DialogContent className="max-w-[90vw] max-h-[90vh] overflow-auto">
                                  ...
                                  <Table>
                                    <TableHeader>
                                      <TableRow>
                                        {m.tabular_data[0] && Object.keys(m.tabular_data[0]).map((key) => (
                                          <TableHead key={key}>{key}</TableHead>
                                        ))}
                                      </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                      {m.tabular_data.filter(row => row && typeof row === 'object').map((row, rowIndex) => (
                                        <TableRow key={rowIndex}>
                                          {Object.values(row).map((value, cellIndex) => (
                                            <TableCell key={cellIndex}>{value !== null && value !== undefined ? String(value) : ''}</TableCell>
                                          ))}
                                        </TableRow>
                                      ))}
                                    </TableBody>
